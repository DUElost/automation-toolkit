...
public class SupbookRecord extends WritableRecordData {
    ...
    private void initExternal() {
        ...
        byte[] bArr = new byte[encodedURL.length + 6 + (i4 * 3) + (i2 * 2)];
        this.data = bArr;
        ...
        while (true) {
            ...
            IntegerHelper.getTwoBytes(strArr[i].length(), this.data, length);
            byte[] bArr3 = this.data;
            bArr3[length + 2] = 1;
            StringHelper.getUnicodeBytes(this.sheetNames[i], bArr3, length + 3);
            length += (this.sheetNames[i].length() * 2) + 3;
            i++;
        }
    }
    ...
    public int getSheetIndex(String str) {
        ...
        strArr2[this.sheetNames.length] = str;
