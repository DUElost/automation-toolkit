...
<#
.SYNOPSIS
    同步 sleeptest 源码到 ATTS，编译并 platform 签名，输出到 stability_Sleep-Test/apk/
#>
param(
...
$TestApkDir = Join-Path $AndroidToolsRoot "stability_Sleep-Test\apk"
...
function Resolve-BuildJavaHome {
    ...
    throw "未找到 Java 11+。请在 app-config.properties 设置 java.home"
}
...
function Sync-SleepTestSources {
    param([string]$AttsDir)
    Write-Step "同步 sleeptest 源码到 ATTS"
    $srcDir = Join-Path $ManagerDir "sleep-apk-src"
    $destJava = Join-Path $AttsDir "app\src\main\java\com\mediatek\schpwronoff\sleeptest"
    ...
    $resDir = Join-Path $ManagerDir "sleep-apk-res"
    Copy-Item (Join-Path $resDir "layout\activity_sleep_test.xml") `
        (Join-Path $AttsDir "app\src\main\res\layout\activity_sleep_test.xml") -Force
    Copy-Item (Join-Path $resDir "values\strings_sleep_test.xml") `
        (Join-Path $AttsDir "app\src\main\res\values\strings_sleep_test.xml") -Force
...
Sync-SleepTestSources -AttsDir $attsDir
...
Write-Host "`n下一步: cd stability_Sleep-Test\scripts && deploy.bat"
