...
class CIFSHealthProbe:
    ...
    def is_mount_healthy(self):
        ...
        try:
            ...
            APP_LOGGER.error(
                f"CIFS 挂载探测超时({self.probe_timeout}s): {self.mount_path}")
        except Exception as e:
...
def _get_aee_log_folder_name(device_id, run_date_stamp):
    try:
        with APP_LOGGER.context(sn=device_id):
            ...
            match_prefix = re.search(r"-(.*?)\(", display_id_prop)
            if match_prefix:
                folder_part_2_prefix = match_prefix.group(1).strip()
            ...
            build_tokens = re.findall(r"\d{6}(?:V\d+)?", post_paren_content, flags=re.IGNORECASE)
            if build_tokens:
                timestamp_from_display = build_tokens[-1]

            # 兼容 MLD-LX3-16-260518V3 这类不带括号的新版本格式
            if not folder_part_2_prefix:
                remaining_display = ""
                ...
                if remaining_display:
                    ...
                    if version_release_prop and display_tokens and display_tokens[0] == version_release_prop:
                        ...
                        if len(display_tokens) > 1 and not timestamp_from_display:
            ...
            su_suffix_source = " ".join(filter(None, [post_paren_content, display_id_prop, version_incremental_id_prop]))
            ...
            if not folder_part_2_prefix and version_incremental_id_prop:
                folder_part_1, folder_part_2_prefix = "", version_incremental_id_prop
                timestamp_from_display = ""
    ...
    try:
        ...
        with APP_LOGGER.context(tag=f"LogProcessor", sn=device_id):
            ...
            for remote_aee_path in AEE_PATHS_ON_DEVICE:
                ...
                try:
                    with open(os.path.join(output_subdir, "db_save_org.txt"), 'w', encoding='utf-8') as f:
                        f.write(history_content)
                except IOError as e:
                    APP_LOGGER.error(f"写入 db_save_org.txt 失败: {e}")
                ...
                try:
                    with open(os.path.join(output_subdir, "db_save.txt"), 'w', encoding='utf-8') as f:
                        f.write('\n'.join(passed_filter_lines))
                except IOError as e:
                    APP_LOGGER.error(f"写入 db_save.txt 失败: {e}")
                ...
                with APP_LOGGER.context(tag="ScanCycle"):
                    ...
                    if not IS_WINDOWS and cifs_probe and CIFS_PROTECTION_ENABLED:
                        if not cifs_probe.is_mount_healthy():
                            APP_LOGGER.error("CIFS 挂载不健康，尝试恢复...")
