...
def test_fill_target_dir_is_on_data_partition():
  assert FILL_TARGET_DIR.startswith("/data/")


def test_build_fill_file_path_uses_data_partition():
  path = _build_fill_file_path("abc123")
  assert path.startswith("/data/")
  assert path.endswith("fill_file_abc123.tmp")


def test_get_storage_info_queries_data_partition():
  fake_output = "Filesystem     1K-blocks    Used Available Use% Mounted on\n/dev/block  10000000 2000000 8000000  20% /data\n"
...
def test_fill_in_chunks_writes_to_data_partition():
  ...
  assert "fill_file_1710000000_5555.tmp" in command
