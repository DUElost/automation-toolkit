...
CIFS_UMOUNT_TIMEOUT = _cifs_prot.get("umount_timeout", 15)
CIFS_MAX_STALE_HEARTBEAT = _cifs_prot.get("max_stale_heartbeat_seconds", 600)
CIFS_REMOUNT_REASONS = frozenset({"mount_missing", "stat_fail"})
def get_local_ip():
    """获取本机IP地址"""
    try:
...
def _run_subprocess_hard_timeout(cmd, timeout, label):
    """运行子进程并在超时后强制终止，避免 umount 等操作无限阻塞主线程。

    Returns:
        (returncode, stdout, stderr, hung): hung=True 表示 kill 后仍未退出。
    """
    APP_LOGGER.info(f"CIFS 恢复: 开始 {label}: {' '.join(cmd)}")
    ...
    try:
        ...
        APP_LOGGER.info(f"CIFS 恢复: {label} 完成, rc={proc.returncode}")
        return proc.returncode, stdout, stderr, False
    except subprocess.TimeoutExpired:
        APP_LOGGER.error(f"CIFS 恢复: {label} 超时({timeout}s)，强制终止子进程")
        try:
        ...
        try:
            ...
            APP_LOGGER.error(
                f"CIFS 恢复: {label} 子进程 kill 后仍未退出，放弃后续 remount")
            try:
                proc.kill()
            except ProcessLookupError:
                pass
...
class CIFSHealthProbe:
    ...
    def _probe_write_access(self):
        ...
        Returns:
            (ok, failure_kind): failure_kind 为 None / 'timeout' / 'failed'
        """
        ...
        try:
            ...
            if result.returncode == 0:
                return True, None
            APP_LOGGER.error(
                f"CIFS 写探针失败(rc={result.returncode}): {self.mount_path}")
            return False, "failed"
        except subprocess.TimeoutExpired:
            APP_LOGGER.error(
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if (
                        ...
                        write_ok, write_failure = self._probe_write_access()
                    ...
                    if not check_mount_status(self.mount_path):
                        ...
                        APP_LOGGER.error(
                            f"CIFS 挂载点不在 mount 列表中: {self.mount_path}")
                    else:
                        self.last_failure_reason = "stat_fail"
                        APP_LOGGER.error(
    ...
    def try_recover_mount(self, target_path, server_ip,
        ...
        """lazy umount + remount；仅在 mount_missing/stat_fail 时由主循环调用。"""
        ...
        APP_LOGGER.warning(f"尝试恢复 CIFS 挂载: {target_path}")
        try:
            mount_present = check_mount_status(target_path)
            if mount_present:
                _, _, stderr, hung = _run_subprocess_hard_timeout(
                ...
                if hung:
                    APP_LOGGER.error(
                        "CIFS 恢复: umount 无法终止，放弃 remount 以防主线程卡死")
                    self.last_failure_reason = "io_slow"
                    return False
                time.sleep(2)
            else:
                APP_LOGGER.info("CIFS 恢复: 挂载点未列出，跳过 umount")

            APP_LOGGER.info(f"CIFS 恢复: 开始 remount: {target_path}")
            ...
            if recovered:
                ...
                APP_LOGGER.error(f"CIFS 恢复: remount 失败: {target_path}")
            return recovered
        except Exception as e:
...
def _safe_makedirs(path, timeout=None):
    """Linux 下用子进程 mkdir -p，避免 Python 线程直接卡死文件系统"""
    timeout = timeout or _CIFS_IO_TIMEOUT
    try:
        if IS_WINDOWS:
        ...
        APP_LOGGER.warning(f"makedirs 超时({timeout}s): {path}")
        return False
    except Exception as e:
        APP_LOGGER.error(f"创建目录失败 {path}: {e}")
        return False
...
def _safe_write_text(path, content, timeout=None):
    ...
    try:
        ...
        APP_LOGGER.warning(f"write_text 超时({timeout}s): {path}")
        return False
    except Exception as e:
        APP_LOGGER.error(f"写文本失败 {path}: {e}")
        return False
    finally:
        try:
            if fd is not None:
...
def _persist_runtime_cache(reason="", force=False):
    """将待处理任务尽快落盘，同时通过短时间窗口合并保存，降低高频写盘开销"""
    ...
    try:
        ...
        if persist_reason:
            APP_LOGGER.info(f"已将待处理缓存落盘: {persist_reason}")
        return True
    except Exception as e:
        with _runtime_cache_persist_lock:
            if persist_reason:
                _runtime_cache_pending_reason = persist_reason
        APP_LOGGER.error(f"落盘待处理缓存失败 {persist_reason}: {e}")
        return False


def _ensure_output_subdir_ready(path):
    """写入 CIFS 上的目标目录前先健康检查。"""
    if IS_WINDOWS:
        return _safe_makedirs(path)
    if _RUNTIME_CIFS_PROBE and not _RUNTIME_CIFS_PROBE.is_mount_healthy():
        reason = _RUNTIME_CIFS_PROBE.last_failure_reason or "unknown"
        APP_LOGGER.warning(
            f"CIFS 不可用(reason={reason})，暂时不写入目录: {path}")
        return False
    return _safe_makedirs(path)
                ...
                with APP_LOGGER.context(tag="ScanCycle"):
                    ...
                    if not IS_WINDOWS and cifs_probe and CIFS_PROTECTION_ENABLED:
                        if not cifs_probe.is_mount_healthy():
                            ...
                            # I/O 缓慢/磁盘满/写失败：退避，禁止 remount
                            if reason in ("io_slow", "disk_full", "write_fail", "write_readonly"):
                                backoff_secs = min(600, 60 * (cifs_probe._consecutive_failures + 1))
                                APP_LOGGER.warning(
                                    f"CIFS 不健康 (reason={reason})，"
                                    f"重挂载无法解决此问题，跳过本轮，退避 {backoff_secs}s")
                                time.sleep(backoff_secs)
                                continue

                            # 仅 mount 丢失或 stat 失败时尝试 remount
                            if reason in CIFS_REMOUNT_REASONS:
                                APP_LOGGER.error(
                                    f"CIFS 挂载异常 (reason={reason})，尝试恢复...")
                            ...
                            backoff_secs = min(600, 60 * (cifs_probe._consecutive_failures + 1))
                            APP_LOGGER.warning(
                                f"CIFS 不健康 (reason={reason})，跳过重挂载，退避 {backoff_secs}s")
                            time.sleep(backoff_secs)
                            continue
