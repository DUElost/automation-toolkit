...
# Embedded file name: modules\analyse\tne\types\tne00160002.py
import os
...
class Tne00160002(TneBase):

    def __init__(self, path):
        super(Tne00160002, self).__init__(path)
        self._exp_class = "0x00160002"
        self._exp_type = "0x00160002"
    ...
    def analyse(self):
        ...
        crash_type = androidCrashLog.analyse_je_or_ne()
        if crash_type == "je":
            self._exp_time, self._detail, self._cur_process = androidCrashLog.analyse_tne07()
            self._exp_class = "0x00160002_JE"
        else:
            if crash_type == "ne":
                self._exp_time, self._detail, self._cur_process = androidCrashLog.analyse_tne08()
                self._exp_class = "0x00160002_NE"
            else:
                TEST_LOGGER.debug(("未知的错误类型，请检查android_crash_log文件：{}".format(self._android_crash_log_path)), tag=(self.tne_dir_name))
                self._exp_type_set.add("TNE_ERROR_unknown_je_ne_in_android_crash_log")
        self._exp_type = self._exp_class
        if not os.path.isfile(self._android_main_log_path):
            ...
            self._package = pkg if pkg else self._cur_process
        if self._exp_type_set:
            self._exp_type = self._exp_type + "\n" + "\n".join(self._exp_type_set)
        self._detail_to_random()
        return True

# okay decompiling extracted_pyz_modules\modules\analyse\tne\types\tne00160002.pyc
