...
def normalize_caused_by_signature(text: Any) -> str:
    """归一化 CausedBy 签名：去掉内存地址、行号、时间戳等噪声"""
    text = str(clean_cell_value(text) or "").strip()
    if not text:
        return ""
    # 1. 去除内存地址 0x00000000abcdef12
    text = re.sub(r"0x[0-9a-fA-F]{8,16}", "0x<ADDR>", text)
    # 2. 去除 Java/C++ 行号 :1234
    text = re.sub(r":\d+", "", text)
    # 3. 去除时间戳 2026-03-29 12:00:00 (粗略匹配)
    text = re.sub(r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}", "[TIMESTAMP]", text)
    # 4. 去除 PID/TID ( 1234)
    text = re.sub(r"\(\s*\d+\)", "(PID)", text)
    # 5. 合并连续空白
...
def prepare_issue_record(
    ...
    return {
        ...
        "Caused by Signature": normalize_caused_by_signature(issue_data["caused_by"]),
    }
