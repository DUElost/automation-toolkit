...
# 1. 配置你的 JIRA 信息
# ==========================================================
JIRA_SERVER = 'http://jira-ex.transsion.com:6001'  # 你的 JIRA 服务器地址
JIRA_USER = 'dailv.tinno'              # 你的邮箱
JIRA_PASSWORD = '2j4-Gx6-beJ-x36'        # <-- 在这里输入你的JIRA登录密码
...
def get_all_projects(client):
    ...
    try:
        ...
        for project in projects:
            print(f"  - Key: {project.key}, 名称: {project.name}")
            
        return projects
        
    except JIRAError as e:
        print(f"[失败] 获取项目列表失败: {e.text} (状态码: {e.status_code})")
        return []
            ...
            for issue_type in project_meta['issuetypes']:
                ...
                for field_id, field_info in issue_type['fields'].items():
                    ...
                    if 'allowedValues' in field_info:
                        allowed_values = [val.get('value', val.get('name', str(val))) for val in field_info['allowedValues'][:5]]  # 只显示前5个
                        print(f"      可选值: {', '.join(allowed_values)}{'...' if len(field_info['allowedValues']) > 5 else ''}")
...
if __name__ == "__main__":
    ...
    if jira:
        ...
        if projects:
            ...
            print(f"\n选择项目 '{first_project.key}' 来查看字段信息...")
            get_project_fields(jira, first_project.key)
            
            # 如果有多个项目，也可以查看其他项目
            if len(projects) > 1:
                print(f"\n\n如需查看其他项目字段，可以修改脚本中的 project_key 变量")
                print(f"可用的项目Keys: {[p.key for p in projects[:5]]}")
