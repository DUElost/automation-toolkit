...
class RegressionConfig:
    ...
    enable_duplicate_followups: bool = False
    write_audit_report: bool = True
    strict_version_project_keys: List[str] = field(default_factory=list)


@dataclass
class OutputConfig:
    sqlite_path: str = "result/tinno_regression_cache.db"
...
def load_regression_rules(path: Union[str, Path]) -> RegressionRules:
    ...
    regression = RegressionConfig(
        ...
        enable_duplicate_followups=_coerce_bool(
            regression_data.get("enable_duplicate_followups"),
            "regression.enable_duplicate_followups",
            False,
        ),
        write_audit_report=_coerce_bool(
            regression_data.get("write_audit_report"),
            "regression.write_audit_report",
            True,
        ),
        strict_version_project_keys=_coerce_str_list(
            regression_data.get("strict_version_project_keys", []),
            "regression.strict_version_project_keys",
            allow_empty=True,
        ),
    )
    ...
    output = OutputConfig(
        sqlite_path=_coerce_str(
            ...
            "output.sqlite_path",
            "result/tinno_regression_cache.db",
        ),
