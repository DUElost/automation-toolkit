...
示例：
    python ftp_reader_20260626.py list /
    python ftp_reader_20260626.py read /remote/path/demo.txt --file-encoding utf-8
    python ftp_reader_20260626.py download /remote/path/demo.zip .\\demo.zip
"""
...
logger = logging.getLogger("ftp_reader")


def setup_debug(enable: bool) -> None:
    if enable:
        logging.basicConfig(
            level=logging.DEBUG,
...
def ftp_connection(config: FTPConfig) -> Generator[FTP, None, None]:
    ...
    logger.debug("正在连接 FTP %s:%s (用户: %s, 超时: %ss, 编码: %s)",
                 config.host, config.port, config.username, config.timeout, config.ftp_encoding)
    ftp.connect(config.host, config.port, timeout=config.timeout)
    ftp.login(config.username, config.password)
    logger.debug("FTP 连接成功: %s:%s", config.host, config.port)
    try:
        yield ftp
    finally:
        try:
...
def format_upload_message(result: UploadResult) -> str:
    if result.path_type == "directory":
        return (
            f"上传完成：{result.local_path} -> {result.remote_path} "
            f"(目录 {result.dir_count} 个，文件 {result.file_count} 个)"
        )
    return f"上传完成：{result.local_path} -> {result.remote_path}"
...
def upload_single_file(
    ...
    ensure_remote_dir(ftp, split_remote_path(remote_file)[0])
    with local_file.open("rb") as fh:
        if progress is not None:
            def _cb(data: bytes) -> None:
                progress.add_bytes(len(data))
            ftp.storbinary(f"STOR {remote_file}", fh, blocksize=DOWNLOAD_BLOCKSIZE, callback=_cb)
        else:
            ftp.storbinary(f"STOR {remote_file}", fh, blocksize=DOWNLOAD_BLOCKSIZE)
...
def upload_directory(
    ...
    for item in sorted(local_dir.iterdir()):
        child_remote = join_remote_path(remote_dir, item.name)
        if item.is_dir():
            child_fc, child_dc = upload_directory(ftp, item, child_remote, progress)
            file_count += child_fc
            dir_count += child_dc
        elif item.is_file():
            upload_single_file(ftp, item, child_remote, progress)
            file_count += 1
            if progress is not None:
                progress.add_file(item.stat().st_size)
    return file_count, dir_count
...
def upload_path(
    ...
    if local.is_dir():
        file_count, dir_count = upload_directory(ftp, local, remote_path, progress)
        return UploadResult(
            local_path=local,
    ...
    if local.is_file():
        upload_single_file(ftp, local, remote_path, progress)
        return UploadResult(
            local_path=local,
            remote_path=remote_path,
            path_type="file",
    ...
    raise RuntimeError(f"不支持的本地路径类型：{local}")
...
def resolve_command_target(
    ...
    if normalized_command == "upload":
        return resolve_ftp_target(config, getattr(args, "remote_path", None) or "")
    return config, "/", None
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="启用 debug 日志输出，在 FTP 操作失败时打印详细错误信息",
    )
    ...
    upload_parser = subparsers.add_parser("upload", aliases=["put"], help="上传本地文件或目录到 FTP")
    upload_parser.add_argument("local_path", help="本地文件或目录路径")
    upload_parser.add_argument(
        "remote_path",
        nargs="?",
        help="远程目标路径，默认使用本地文件/目录名；支持直接传入 主机/路径 或 ftp://主机/路径",
    )
...
def safe_main() -> int:
    ...
    try:
        ...
        if normalized_command == "upload":
            ...
            remote_path = resolved_target if resolved_target != "/" else None
            workers = getattr(args, 'workers', DEFAULT_MAX_WORKERS)

            def _do_upload(ftp: FTP) -> UploadResult:
                return upload_path(ftp, local_path, remote_path)

            upload_result, _ = run_ftp_operation(config, _do_upload)

            if upload_result.path_type == "directory" and upload_result.file_count > 0:
                print(
                    f"[upload] {upload_result.file_count} files -> {upload_result.remote_path}",
                    file=sys.stderr,
                )
            print(format_upload_message(upload_result))
            return 0

        parser.print_help()
        return 1
    except RuntimeError as exc:
        logger.debug("FTP 操作失败 (RuntimeError): %s\n%s", exc, traceback.format_exc())
        print(f"处理失败：{exc}", file=sys.stderr)
        return 1
    except socket.timeout as exc:
        logger.debug(
            "FTP 操作超时 | 主机: %s:%s | 用户: %s | 超时设置: %ss | 错误: %s\n%s",
            config.host, config.port, config.username, config.timeout,
            exc, traceback.format_exc(),
        )
        print(
            f"FTP 操作超时（{config.timeout}s）：连接 {config.host}:{config.port} 失败，"
            f"请检查网络或防火墙设置。详细错误：{exc}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        with ftp_connection(config) as ftp:
            ...
            if normalized_command == "upload":
                local_path = Path(args.local_path)
                remote_path = resolved_target if resolved_target != "/" else None
                upload_result = upload_path(ftp, local_path, remote_path)
                print(format_upload_message(upload_result))
                return 0
