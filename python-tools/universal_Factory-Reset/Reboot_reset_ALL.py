...
def parse_arguments():
    ...
    parser.add_argument(
        "-m", "--mode",
        help="指定测试模式编号 (1-6)，或使用 all 表示顺序执行全部模式（等同于选项4）"
    )
    parser.add_argument(
        "-l", "--loop", type=int,
...
if __name__ == "__main__":
    ...
    if args.mode:
        ...
        if mode_arg.lower() == "all":
            ...
            try:
                ...
                print(f"{Fore.RED}无效的模式参数: {mode_arg}，请使用1-6或all。{Style.RESET_ALL}")
                pause_before_exit(args.no_pause)
                sys.exit(1)
        if str(switch_value) not in CONFIG['test_modes']:
            print(f"{Fore.RED}模式 {mode_arg} 不在配置范围内（有效值1-6或all）。{Style.RESET_ALL}")
            pause_before_exit(args.no_pause)
            sys.exit(1)
    ...
    if switch_value is None:
        ...
        switch_value = int(input(f"\n{Fore.CYAN}请输入测试项编号 (1-6)：{Style.RESET_ALL}"))
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(handle_device, device, loop_count, switch_value) for device in devices]
